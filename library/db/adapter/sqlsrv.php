 ...
 * @package database
 */
class QDB_Adapter_Sqlsrv extends QDB_Adapter_Abstract {
    ...
    function connect($pconnect = false, $force_new = false) {
        ...
        $conntectOption = array(
            'UID' => $this->_dsn['login'],
            'PWD' => $this->_dsn['password'],
            'ReturnDatesAsStrings' => true,
        );
        if (!empty($this->_dsn['database'])) {
            $conntectOption['Database'] = $this->_dsn['database'];
        ...
        if (isset($this->_dsn['charset']) && $this->_dsn['charset'] != '') {
            ...
            $conntectOption['CharacterSet'] = $charset;
        }
        $this->_conn = sqlsrv_connect($host, $conntectOption);
        if (!is_resource($this->_conn)) {
            throw new QDB_Exception('CONNECT DATABASE', print_r(sqlsrv_errors(), true));
        }
    }
    ...
    function insertID() {
        return sqlsrv_insert_id($this->_conn);
    }

    function affectedRows() {
        return sqlsrv_rows_affected($this->_lastrs);
    }
    ...
    function execute($sql, $inputarr = null) {
        ...
        if (is_resource($result)) {
            return new QDB_Result_Sqlsrv($result, $this->_fetch_mode);
        } elseif ($result) {
            $this->_last_err = null;
            $this->_last_err_code = null;
            return $result;
        } else {
            $this->_last_err = print_r(sqlsrv_errors(), true);
